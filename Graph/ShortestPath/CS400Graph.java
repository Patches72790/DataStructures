...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            // make new objects of start and end from source
            this.start = new Vertex(source.start.data);
            this.distance = source.distance;
            this.end = new Vertex(source.end.data);
            this.dataSequence = new LinkedList<>();

            // add all entries of objects from source list
            this.dataSequence.addAll(source.dataSequence);

            // make new copies of start vertex edges
            for (Edge edge : source.start.edgesLeaving) {
                this.start.edgesLeaving.add(new Edge(edge.target, edge.weight));
            }

            // make new copies of end vertex edges
            for (Edge edge : source.end.edgesLeaving) {
                this.end.edgesLeaving.add(new Edge(edge.target, edge.weight));
            }
        }
        ...
        public void extend(Edge edge) {
            // update the total weight of path
            this.distance += edge.weight;

            // add new vertex data to sequence
            dataSequence.add(edge.target.data);

            // update the end of path to new vertex
            end = edge.target;
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        // priority queue for managing the paths in the graph
        PriorityQueue<Path> frontier = new PriorityQueue<>();

        // get ref to starting vertex
        Vertex startingVertex = vertices.get(start);

        // if start doesn't exist, throw exception
        if (startingVertex == null) {
            throw new NoSuchElementException("No path from start " + start + " to end " + end  + " could be found!");
        }

        // add start to path
        Path shortestPath = new Path(startingVertex);

        // add starting vertex to priority queue
        frontier.add(shortestPath);

        // invariant -- frontier has vertices
        while (!frontier.isEmpty()) {

            // get the least path
            Path leastPath = frontier.poll();

            // update current shortest path candidate
            shortestPath = leastPath;

            // if the end has been reached or the length of shortest path reaches number of vertices, finished
            if (shortestPath.end.data.equals(end) || (shortestPath.dataSequence.size() >= getVertexCount()) ) {
                break;
            }

            // add its neighbors to frontier
            for (Edge edge : leastPath.end.edgesLeaving) {

                // don't add edge that is already in the shortest path
                if (shortestPath.dataSequence.contains(edge.target.data)) {
                    continue;
                }

                // otherwise, add the new path into frontier
                Path nextPath = new Path(shortestPath);
                nextPath.extend(edge);
                frontier.add(nextPath);
            }
        }

        // throw exception if the end was never found
